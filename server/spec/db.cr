...
describe MoanaDB do
  ...
  it "creates a Cluster" do
    ...
    c2.name.should eq("my_cluster")
    c2.id.should eq(cluster.id)

    c3 = MoanaDB.update_cluster(cluster.id, "my_cluster_renamed")
    c3.id.should eq(cluster.id)
    c3.name.should eq("my_cluster_renamed")
  ...
  it "creates a Node" do
    ...
    n2.id.should eq(node.id)
    n2.hostname.should eq("node1.example.com")
    n2.endpoint.should eq("node1.example.com:5002")

    n3 = MoanaDB.update_node(node.id, hostname: "node2.example.com")
    n3.hostname.should eq("node2.example.com")

    n4 = MoanaDB.update_node(node.id, endpoint: "node2.example.com:5003")
    n4.endpoint.should eq("node2.example.com:5003")
