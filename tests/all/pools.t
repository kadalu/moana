...
# Replicate
TEST "kadalu pool create vol2 replica server1:/exports/vol2/s1 server2:/exports/vol2/s2 server3:/exports/vol2/s3"

# Disperse
TEST "kadalu pool create vol3 data server1:/exports/vol3/s1 server2:/exports/vol3/s2 redundancy server3:/exports/vol3/s3"

# Distributed Replicate
TEST "kadalu pool create vol4 replica server1:/exports/vol4/s1 server2:/exports/vol4/s2 server3:/exports/vol4/s3 replica server1:/exports/vol4/s4 server2:/exports/vol4/s5 server3:/exports/vol4/s6"
...
puts TEST "mount -t kadalu http://#{nodes[0]}:3000:vol2 /mnt/vol2"
...
["vol1", "vol2", "vol3", "vol4"].each do |vol|
  TEST "kadalu pool stop #{vol} --mode=script"
  TEST "kadalu pool delete #{vol} --mode=script"
end
...
USE_NODE nodes[0]
TEST "kadalu pool create vol5 server1:/exports/vol5/s1 server2:/exports/vol5/s2 server3:/exports/vol5/s3 --auto-create-pool --auto-add-nodes"
TEST "kadalu pool stop vol5 --mode=script"
TEST "kadalu pool delete vol5 --mode=script"
...
# Case 1
# Create vol6 & delete it. Reuse the same path with --pool-id for vol7.
USE_NODE nodes[0]
vol_id = create_pool_and_get_id("kadalu pool create vol6 server1:/exports/vol6/s1 server2:/exports/vol6/s2 server3:/exports/vol6/s3 --no-start")
TEST "kadalu pool delete vol6 --mode=script"
new_vol_id = create_pool_and_get_id("kadalu pool create vol7 server1:/exports/vol6/s1 server2:/exports/vol6/s2 server3:/exports/vol6/s3 --no-start --pool-id=#{vol_id}")
EQUAL vol_id, new_vol_id, "Checking if pool-id are equal after vol6 is reused with --pool-id in vol7"
TEST "kadalu pool delete vol7 --mode=script"

# Case 2
# Create vol8. Create vol9 with --pool-id of active vol8.
vol_id = create_pool_and_get_id("kadalu pool create vol8 server1:/exports/vol8/s1 server2:/exports/vol8/s2 server3:/exports/vol8/s3 --no-start")
TEST 1, "kadalu pool create vol9 server1:/exports/vol9/s1 server2:/exports/vol9/s2 server3:/exports/vol9/s3 --no-start --pool-id=#{vol_id}"
TEST "kadalu pool delete vol8 --mode=script"

# Case 3
# Create vol10 with full storage-unit directory
TEST "kadalu pool create vol10 server1:/exports/vol10/s1 --no-start"
TEST "kadalu pool delete vol10 --mode=script"

# Case 4 [Negation of Case1]
# Create vol11 & delete it. Create vol12 with same path of unactive vol11 without --pool-id.
TEST "kadalu pool create vol11 server1:/exports/vol11/s1 server2:/exports/vol11/s2 server3:/exports/vol11/s3 --no-start"
TEST "kadalu pool delete vol11 --mode=script"
TEST 1, "kadalu pool create vol12 server1:/exports/vol11/s1 server2:/exports/vol11/s2 server3:/exports/vol11/s3 --no-start"

# Case 5
# Create vol13 with fresh path & no xattrs using --pool-id with wrong format. [Check for matching format of vol-id with uuid]
TEST 1, "kadalu pool create vol13 server1:/exports/vol12/s1 server2:/exports/vol12/s2 server3:/exports/vol12/s3 --no-start --pool-id=123-456-789"


# Tests for restarting of all services on node-reboot
TEST "kadalu pool create vol14 server1:/exports/vol14/s1 server2:/exports/vol14/s2 server3:/exports/vol14/s3"
nodes.each do |node|
  USE_NODE node
...
USE_NODE nodes[0]
TEST "kadalu pool stop vol14 --mode=script"
TEST "kadalu pool delete vol14 --mode=script"

# Volfile Server tests
# pool create with known Storage unit port
TEST "mkdir -p /exports/vol_volfile/s1"
TEST "kadalu pool create vol_volfile server1:5007:/exports/vol_volfile/s1"
...
puts TEST "mount -t kadalu http://server1:3000:/vol_volfile /mnt/vol_volfile_1"
puts TEST "df /mnt/vol_volfile_1"

# TODO: Mgr URL from ENV
TEST "mkdir -p /mnt/vol_volfile_2"
puts TEST "mount -t kadalu /vol_volfile /mnt/vol_volfile_2"
puts TEST "df /mnt/vol_volfile_2"

# Using Storage Unit URL directly
TEST "mkdir -p /mnt/vol_volfile_3"
puts TEST "mount -t kadalu server1:5007:/vol_volfile /mnt/vol_volfile_3"
puts TEST "df /mnt/vol_volfile_3"

# Using Volfile Server option
TEST "mkdir -p /mnt/vol_volfile_4"
puts TEST "mount -t kadalu -o \"volfile-server=server1:5007\" /vol_volfile /mnt/vol_volfile_4"
puts TEST "df /mnt/vol_volfile_4"

# Using Volfile Servers option
TEST "mkdir -p /mnt/vol_volfile_5"
puts TEST "mount -t kadalu -o \"volfile-servers=server1:5007 server1:5007\" /vol_volfile /mnt/vol_volfile_5"
...
TEST "kadalu pool set vol_volfile diagnostics.client-log-level DEBUG"
TEST "sleep 5"
...
TEST "kadalu pool stop vol_volfile --mode=script"
TEST "kadalu pool delete vol_volfile --mode=script"

# Tests for Backup & Restore
TEST "kadalu pool create vol19 server1:/exports/vol19/s1 server2:/exports/vol19/s2 server3:/exports/vol19/s3"
USE_NODE nodes[0]
...
TEST "kadalu pool stop vol19 --mode=script"
TEST "kadalu pool delete vol19 --mode=script"

# Tests for volume expansion
# Distribute
TEST "kadalu pool create vol15 server1:/exports/vol15/s1 server2:/exports/vol15/s2 server3:/exports/vol15/s3"
TEST "mkdir -p /mnt/vol15"
puts TEST "kadalu mount vol15 /mnt/vol15"
...
TEST "kadalu pool expand vol15 server1:/exports/vol15/s1_e server2:/exports/vol15/s2_e server3:/exports/vol15/s3_e"
TEST "sleep 3"
...
USE_NODE nodes[0]
TEST "kadalu rebalance start vol15"
TEST "sleep 3"
...
TEST "kadalu rebalance status vol15"
TEST "kadalu rebalance status vol15 --detail"

TEST "umount /mnt/vol15"
TEST "rmdir /mnt/vol15"
TEST "kadalu pool stop vol15 --mode=script"
TEST "kadalu pool delete vol15 --mode=script"

# Replicate
TEST "kadalu pool create vol16 replica server1:/exports/vol16/s1 server2:/exports/vol16/s2 server3:/exports/vol16/s3"
TEST "mkdir -p /mnt/vol16"
puts TEST "kadalu mount vol16 /mnt/vol16"
...
TEST "kadalu pool expand vol16 replica server1:/exports/vol16/s1_e server2:/exports/vol16/s2_e server3:/exports/vol16/s3_e"
...
USE_NODE nodes[0]
TEST "kadalu rebalance start vol16"
TEST "sleep 3"
...
TEST "kadalu pool stop vol16 --mode=script"
TEST "kadalu pool delete vol16 --mode=script"

# Distributed Replicate
TEST "kadalu pool create vol17 replica server1:/exports/vol17/s1 server2:/exports/vol17/s2 server3:/exports/vol17/s3 replica server1:/exports/vol17/s4 server2:/exports/vol17/s5 server3:/exports/vol17/s6"
TEST "mkdir -p /mnt/vol17"
puts TEST "kadalu mount vol17 /mnt/vol17"
...
TEST "kadalu pool expand vol17 replica server1:/exports/vol17/s1_e server2:/exports/vol17/s2_e server3:/exports/vol17/s3_e replica server1:/exports/vol17/s4_e server2:/exports/vol17/s5_e server3:/exports/vol17/s6_e"
...
USE_NODE nodes[0]
TEST "kadalu rebalance start vol17"
TEST "sleep 3"
...
TEST "kadalu pool stop vol17 --mode=script"
TEST "kadalu pool delete vol17 --mode=script"

# Disperse
TEST "kadalu pool create vol18 data server1:/exports/vol18/s1 server2:/exports/vol18/s2 redundancy server3:/exports/vol18/s3"
TEST "mkdir -p /mnt/vol18"
puts TEST "kadalu mount vol18 /mnt/vol18"
...
TEST "kadalu pool expand vol18 data server1:/exports/vol18/s1_e server2:/exports/vol18/s2_e redundancy server3:/exports/vol18/s3_e"
...
USE_NODE nodes[0]
TEST "kadalu rebalance start vol18"
TEST "sleep 3"
...
TEST "kadalu pool stop vol18 --mode=script"
TEST "kadalu pool delete vol18 --mode=script"
...
USE_NODE nodes[0]
TEST "kadalu pool create vol20a server1:/exports/vol20a/s1 --no-start"
TEST "kadalu pool create vol20b server1:/exports/vol20b/s1 server2:/exports/vol20b/s2 server3:/exports/vol20b/s3 --no-start"
TEST 1, "kadalu pool rename vol20b vol20b"

TEST "kadalu pool delete vol20a --mode=script"
TEST 0, "kadalu pool rename vol20b vol20b"

puts TEST "kadalu node list"

TEST "kadalu pool delete vol20b --mode=script"
